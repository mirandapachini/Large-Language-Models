"""Streamlit interface for the local NP RAG application."""
...
def show_sources(sources: list[dict]) -> None:
    if not sources:
        return
    with st.expander("Retrieved course sources", expanded=False):
...
st.title("📊 MSBA NP Tutor")
st.markdown(
    "Ask questions about NP concepts, formulas, assumptions, interpretations, "
    "and worked examples. Answers are grounded in the course files stored in `docs/`."
)
...
if not current_files:
    st.info(
        ...
        "A sample NP note is included in the distributed project."
    )
    st.stop()
...
if not st.session_state.messages:
    ...
    examples = [
        "What is ROUGE Score?",
        "What is BLEU",
        "What is Vector Space Dimension?",
        "How does a businessman think about NL?",
        "What is embedding?",
    ]
...
question = st.chat_input("Ask a NP question...")
...
if question:
    ...
    with st.chat_message("assistant"):
        if not ollama_ok:
            ...
            with st.spinner("Retrieving course evidence and generating an explanation..."):
